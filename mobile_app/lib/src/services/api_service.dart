...
class APIService {
  ...
  /// Create API service with certificate pinning enabled
  factory APIService.withPinning({String? baseUrl}) {
    return APIService(
      baseUrl: baseUrl,
  ...
  Future<Map<String, dynamic>> getRealTimeData(String collarId) async {
    final r = await _httpClient.get(Uri.parse('$baseUrl/realtime?collar_id=$collarId'));
  ...
  Future<Map<String, dynamic>> getPetPlan(String collarId) async {
    final r = await _httpClient.get(Uri.parse('$baseUrl/pet-plan?collar_id=$collarId'));
    if (r.statusCode >= 400) throw Exception('Plan error: ${r.statusCode}');
    return json.decode(r.body) as Map<String, dynamic>;
  }

  Future<List<dynamic>> getPetTimeline(String collarId) async {
    final r = await _httpClient.get(Uri.parse('$baseUrl/pet-timeline?collar_id=$collarId'));
  ...
  Future<void> submitFeedback(String eventId, String feedback) async {
    final r = await _httpClient.post(Uri.parse('$baseUrl/submit-feedback'),
